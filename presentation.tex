...
%--------------------------------------------------------------------------
\title{data-driven social dynamics}
\subtitle{two approaches towards socially-aware joint actions}
...
\begin{frame}{'Classical' joint action}
    \begin{itemize}
        \item establishing a joint goal
        \item planning for the robot
        \item planning for the human in order to set the expectations
        \item executing the plans
        \item monitoring progress of the partner towards the goal
    \end{itemize}

    \pause

    \begin{itemize}
        \item joint plans: how to communicate them?
        \item how to deal with the human's own plans?
        \item monitoring/recognising error situations
        \item recognise when we're going 'off track'
    \end{itemize}
\end{frame}
...
\begin{frame}{How do humans perform tasks together?}
    ...
    Humans are good at it. They are also really good at minimizing the cost
    involved.
    
    The key mecanism here is: prefer implict to explicit!
\end{frame}

{\paper{Work by Chris Wallbridge}
...
\begin{frame}{Theoretical foundations: surface alignement; grounding criterion}

    Psycholinguistics provides a lot of the foundational work on these
    questions.

    \begin{itemize}
        \item Communication is a dynamic social process: the partner often to signal
            missing/misunderstood informations
        \item Repairing is generally less costly than avoiding ambiguities in
            the first place
        \item Participants aim at reaching a \emph{grounding criterion}, ie
            \emph{enough} mutual understanding for the task
        \item $\Rightarrow$ we typically reach only partial (or surface)
            alignement -- full alignment is typically not required
    \end{itemize}
\end{frame}
}
...
\begin{frame}{In social Human-Robot interaction}
    ...
    The problem has been studied quite a lot in communication ($\rightarrow$ back-channeling)
...
\begin{frame}{Our roadmap}
    ...
    \begin{itemize}
        \item Do not be scared of ambiguous/partially defined instructions (as
            long as you can recognise grounding errors and requests for
            clarification!)
        \item $\rightarrow$ recognising \emph{grounding errors} should be
            research priority
        \item to reduce the socio-cognitive cost of collaboration, rely as much
            as possible on implicit (sub-conscious) social mechanisms
    \end{itemize}
...
\imageframe[scale=0.9]{sparc/sparc}
...
\imageframe[caption={A diversity of actions selected by the robot},scale=0.9]{sparc/actions}

\imageframe[caption={Take-home message for joint-action}, scale=0.9]{sparc/sparc}
...
\imageframe[scale=0.95,caption=13000+ annotations]{freeplay/coding-scheme}
...
\section{Very early results}

\imageframe[caption=Simple RNN network]{pinsoronet/pinsoro-net-v2}
\imageframe[caption=Trained on 10 epochs x 2M datapoints]{pinsoronet/pinsoronet-v2-accuracy-social-attitude-task-engagement}
